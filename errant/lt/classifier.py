...
def load_pos_map(path):
    ...
    #     map_dict["NFP"] = "X"
    #     map_dict["kita"] = "X"
    #     map_dict['sktv.raid.kiek.mot.Vt.'] = "NUM"
    return map_dict
...
spell = Hunspell("lt_LT_DML6", hunspell_data_dir=str(basename / "resources" / "hunspell"))
# Part of speech map file
pos_map = load_pos_map(base_dir/"resources"/"lt_map")
...
def get_edit_info(toks):
    ...
    dep = []
    for tok in toks:
        pos.append(pos_map.get(tok.tag_, tok.tag_))
        #pos.append(tok.pos_)
        dep.append(tok.dep_)
    return pos, dep
...
def get_two_sided_type(o_toks, c_toks):
    ...
    if len(o_toks) == len(c_toks) == 1:
        ...
        if o_toks[0].text.isalpha():
            ...
            if not spell.spell(o_toks[0].text) and not spell.spell(o_toks[0].lower_):
        ...
        # 3. MORPHOLOGY
        # Only ADJ, ADV, NOUN and VERB can have inflectional changes.# todo: check for lithuanian
        ...
        if stemmer.stemWord(o_toks[0].text) == stemmer.stemWord(c_toks[0].text) and \
                o_pos[0] in open_pos2 and \
                c_pos[0] in open_pos2:
            return "MORPH"
